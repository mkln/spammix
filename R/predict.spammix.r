predict.spammix <- function(object,
                             newx,
                             newcoords, 
                             n_threads=4,
                             verbose=FALSE, ...){
  ...
  if(is.null(object$savedata)){
    stop("Data not saved from spammix output.")
  }
  if(any(object$savedata$family != "gaussian")){
    stop("Currently not implemented. Insert prediction locations into main spammix functions.")
  }
