...
inline double binomial_neghess_mult_sqrt(int npop, const double& exij){
  double opexij = (1.0 + exij);
  return pow(npop * exij / (opexij*opexij), 0.5);
}
...
inline double get_mult(const double& y, //const double& tausq, 
  ...
  if (family == 1){
    ...
    mult = binomial_neghess_mult_sqrt(npop, exij);
  } 
  return mult;
}
...
inline arma::vec get_likdens_likgrad(double& loglike,
  ...
  if(family == 1){ //if(family == "poisson"){
    ...
    Rcpp::Rcout << family << endl;
    Rcpp::stop("[get_likdens_likgrad] wrong family code when updating w. This should be a poisson update\n");
  }
  // old
