...
Rcpp::List meshed_mcmc(
    ...
    bool sample_w=true, bool sample_N=true){
  ...
  MeshedMNM msp(y, X, Z,
                ...
                matern_twonu,
                start_w, beta, gamma, start_lambda, lambda_mask, start_theta, 
  ...
  arma::cube g_mcmc = arma::zeros(Z.n_cols, q, mcmc_thin*mcmc_keep);
  ...
  for(int i=0; i<mcmc_keep; i++){
    ...
    if(!low_mem){
      ...
      npop_mcmc[iname] = Rcpp::wrap(arma::zeros(msp.y.n_rows, q));
    }
  }
  ...
  try {
    ...
    for(m=0; (m<mcmc) & (!interrupted); m++){
      ...
      if(mx >= 0){
        ...
        g_mcmc.slice(w_saved) = msp.gamma;
        ...
        if(mx % mcmc_thin == 0){
          ...
          if(!low_mem){
            ...
            npop_mcmc[iname] = Rcpp::wrap(msp.Npop);
          }
    ...
    return Rcpp::List::create(
      ...
      Rcpp::Named("npop_mcmc") = npop_mcmc,
      Rcpp::Named("beta_mcmc") = b_mcmc,
      Rcpp::Named("gamma_mcmc") = g_mcmc,
